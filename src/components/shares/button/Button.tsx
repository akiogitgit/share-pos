...
export const Button: FC<Props> = ({
  ...
  // bg-primary-dark border-primary-dark text-primary-dark
  ...
  const colorClass = useMemo(() => {
    switch (variant) {
      ...
      case 'light':
        return `bg-${color}-light text-${color} border-1 border-${color}-light`
      case 'outline':
        return `bg-white text-${color} border-1 border-${color}`
      case 'default':
        return `shadow-md shadow-${color}/20 ring-1 ring-${color}-light/40 text-${color}`
    }
  }, [color, variant])
